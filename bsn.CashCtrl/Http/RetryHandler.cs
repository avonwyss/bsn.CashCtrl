		...
		private class RetryableHttpContent: HttpContent {
			...
			public async ValueTask AssertBufferStream() {
				if (bufferStream == null) {
					...
					await Original.CopyToAsync(bufferStream, null).ConfigureAwait(false);
					bufferStream.Seek(0, SeekOrigin.Begin);
				}
			}

			protected override async Task<Stream> CreateContentReadStreamAsync() {
				await AssertBufferStream().ConfigureAwait(false);
				return bufferStream;
			}
			...
			protected override async Task SerializeToStreamAsync(Stream stream, TransportContext context) {
				await AssertBufferStream().ConfigureAwait(false);
				await stream.WriteAsync(bufferStream.GetBuffer(), 0, (int)bufferStream.Length).ConfigureAwait(false);
			}
				...
				do {
					try {
						...
						case HttpStatusCode.ServiceUnavailable:
							...
							await Delay(sync, RetryUnavailableMilliseconds, cancellationToken).ConfigureAwait(false);
							break;
						case (HttpStatusCode)429:
							...
							await Delay(sync, RetryTooManyRequestsMilliseconds, cancellationToken).ConfigureAwait(false);
							break;
						default:
						...
						await Delay(sync, RetryExceptionMilliseconds, cancellationToken).ConfigureAwait(false);
					}
